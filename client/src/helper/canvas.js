...
export function getFocuseDemention(element, padding) {
  ...
  if (element.tool == "line" || element.tool == "arrow") return { fx: x1, fy: y1, fw: x2, fh: y2 }
...
function rgba(color, opacity) {
  ...
  if (!matches) {
    throw new Error("Invalid color format. Please provide a color in RGBA format.")
  }
  opacity /= 100
  const red = Number.parseInt(matches[1])
  const green = Number.parseInt(matches[2])
  const blue = Number.parseInt(matches[3])
  const alpha = Number.parseFloat(matches[4] * opacity || opacity)
