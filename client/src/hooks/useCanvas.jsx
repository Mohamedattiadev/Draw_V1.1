...
import { draw, drawFocuse, cornerCursor, inSelectedCorner } from "../helper/canvas"
import {
  adjustCoordinates,
  arrowMove,
  createElement,
...
export default function useCanvas() {
  ...
  const handleMouseMove = (event) => {
    ...
    if (selectedElement) {
      setInCorner(inSelectedCorner(getElementById(selectedElement.id, elements), clientX, clientY, padding, scale))
    }
    ...
    if (action == "draw") {
      ...
      updateElement(id, { x2: clientX, y2: clientY }, setElements, elements, true)
    } else if (action == "move") {
  ...
  const handleMouseUp = (event) => {
    ...
    if (action.startsWith("resize")) {
      const { id, x1, y1, x2, y2 } = adjustCoordinates(getElementById(selectedElement.id, elements))
      updateElement(id, { x1, x2, y1, y2 }, setElements, elements, true)
    }
  }
  ...
  useEffect(() => {
    ...
    } else if ((keys.has(" ") || selectedTool == "hand") && action != "move" && action != "resize") {
